window.addEventListener("load", () => {
  ...
  function spawnWave(){
    ...
    let baseTier;
    if(player.level >= 40) baseTier = 5; // FINALBOSS somente no level 40
    ...
    for(let i=0;i<count;i++){
      ...
      if(baseTier < 5){
        if(Math.random()<0.12) varTier = Math.max(1, baseTier-1);
        // Nunca sobe para 5 antes do lvl 40
      }
  ...
  function draw(){
    ...
    if(!pendingUpgrade){
      ...
      if(player.sprite.complete && player.sprite.naturalWidth>0) ctx.drawImage(player.sprite,player.x,player.y,player.w,player.h);
